...
impl<'a> Executor<'a> {
    ...
    fn schedule(&self) -> impl Fn(Runnable) + Send + Sync + 'static {
        let state = self.state().clone();
        //log::info!("schedule queue {} s {:?}",state.queue.type_str(),std::thread::current().name());
        // TODO(stjepang): If possible, push into the current local queue and notify the ticker.
        move |runnable| {
            state.queue.reset_to_unbounded();
            match state.queue.push(runnable) {
                Ok(_) => {}
                Err(e) => {
                    if e.is_full() {
                        log::info!(
                            "queue error full len {} cap {:?} type {} s {:?}",
                            state.queue.len(),
                            state.queue.capacity(),
                            state.queue.type_str(),
                            std::thread::current().name()
                        );
                    } else {
                        log::info!("push queue error {:?}", e);
                    }
                }
            };
            state.notify();
...
pub struct LocalExecutor<'a> {
    /// The inner executor.
    inner: once_cell::sync::OnceCell<Executor<'a>>,
        ...
        move |runnable| {
            match state.queue.push(runnable) {
                ...
                Err(e) => {
                    if e.is_full() {
                        log::info!(
                            "push queue error is full len {} cap {:?} type {}",
                            state.queue.len(),
                            state.queue.capacity(),
                            state.queue.type_str()
                        );
...
impl State {
    ...
    fn notify(&self) {
        ...
        // if !self.notified {
        //     unsafe {
        //         *(&self.notified as *const _ as *mut _) = true;
        //     }
        //     let waker = self.sleepers.lock().unwrap().notify();
        //     if let Some(w) = waker {
        //         w.wake();
...
impl Ticker<'_> {
    ...
    fn sleep(&self, waker: &Waker) -> bool {
        ...
        // match self.sleeping {
        //     0 => unsafe {
        //         *(&self.sleeping as *const _ as *mut _) = sleepers.insert(waker);
        //     },
        //     id => {
        //         if !sleepers.update(id, waker) {
        //             return false;
        ...
        unsafe {
            *(self.state as *const _ as *mut _) = sleepers.is_notified();
        }
        // self.state
        //     .notified
    ...
    fn wake(&self) {
        ...
        // let id = unsafe {
        //     let old = self.sleeping;
        //     *(&self.sleeping as *const _ as *mut _) = 0;
        //     old
        // };
        if id != 0 {
            let mut sleepers = self.state.sleepers.lock().unwrap();
            ...
            //     .swap(sleepers.is_notified(), Ordering::SeqCst);
            unsafe {
                *(&self.state.notified as *const _ as *mut _) = sleepers.is_notified();
            }
        }
    }
...
impl Runner<'_> {
    ...
    async fn runnable(&self) -> Runnable {
        ...
        //     let old = self.ticks;
        //     let next = old + 1;
        //     *(&self.ticks as *const _ as *mut _) = next;
        //     old
        // };
        if ticks % 64 == 0 {
...
fn steal<T>(src: &ConcurrentQueue<T>, dest: &ConcurrentQueue<T>) {
    ...
    if count > 0 {
        ...
        for _ in 0..count {
            if let Ok(t) = src.pop() {
                if !dest.push(t).is_ok() {
                    log::info!("dest queue is full");
                }
            } else {
                break;
            }
