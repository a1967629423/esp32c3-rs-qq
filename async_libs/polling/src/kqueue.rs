...
impl Poller {
    ...
    pub fn modify(&self, fd: RawFd, ev: Event) -> io::Result<()> {
        if fd != self.read_stream.as_raw_fd() {
            log::info!("add: kqueue_fd={}, fd={}, ev={:?}", self.kqueue_fd, fd, ev);
        }
    ...
    pub fn wait(&self, events: &mut Events, timeout: Option<Duration>) -> io::Result<()> {
        log::info!("wait: kqueue_fd={}, timeout={:?}", self.kqueue_fd, timeout);
        ...
        log::info!("new events: kqueue_fd={}, res={}", self.kqueue_fd, res);
    ...
    pub fn notify(&self) -> io::Result<()> {
        log::info!("notify: kqueue_fd={}", self.kqueue_fd);
...
impl Drop for Poller {
    fn drop(&mut self) {
        log::info!("drop: kqueue_fd={}", self.kqueue_fd);
