...
static mut BLOCK_ON_COUNT: usize = 0;
    ...
    loop {
        ...
        if last_tick == tick {
            ...
            if let Some(mut reactor_lock) = reactor_lock {
                log::info!("main_loop: waiting on I/O");
...
pub fn block_on<T>(future: impl Future<Output = T>) -> T {
    ...
    let waker = waker_fn({
        ...
        move || {
            if u.unpark() {
                ...
                //     Reactor::get().notify();
                // }
                if !IO_POLLING.with(Cell::get) && *io_blocked {
                    Reactor::get().notify()
                }
            }
        }
    ...
    loop {
        // Poll the future.
        log::info!("pool the future");
        if let Poll::Ready(t) = future.as_mut().poll(cx) {
            log::info!("block_on: completed");
            return t;
        }
        log::info!("pool complete");

        // Check if a notification was received.
        if p.park_timeout(Duration::from_secs(0)) {
            log::info!("block_on: notified");
        ...
        if let Some(mut reactor_lock) = Reactor::get().try_lock() {
            ...
            loop {
                ...
                unsafe { *(&io_blocked as *const _ as *mut _) = true }
                let _guard = CallOnDrop(|| {
                    IO_POLLING.with(|io| io.set(false));
                    // io_blocked.store(false, Ordering::SeqCst);
                    unsafe {
                        *(&io_blocked as *const _ as *mut _) = false;
                    }
                });
                ...
                // Wait for I/O events.
                log::info!("block_on: waiting on I/O");
                reactor_lock.react(None).ok();
                ...
                if start.elapsed() > Duration::from_micros(500) {
                    log::info!("block_on: stops hogging the reactor");
            ...
            log::info!("block_on: sleep until notification");
            p.park();
        }
    }
}
