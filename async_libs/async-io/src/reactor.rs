...
impl Reactor {
    ...
    pub(crate) fn insert_timer(&self, when: Instant, waker: &Waker) -> usize {
        ...
        log::info!("insert timer id {}", id);
        // let id = unsafe {
        //     let source = ID_GENERATOR;
        //     ID_GENERATOR += 1;
        //     source
        // };
            ...
            .is_err()
        {
            ...
            log::info!("timer push error");
            let mut timers = self.timers.lock().unwrap();
            self.process_timer_ops(&mut timers);
        }
        log::info!("timer push success");
    ...
    fn process_timers(&self, wakers: &mut Vec<Waker>) -> Option<Duration> {
        ...
        log::info!("process_timers: {} ready wakers", ready.len());
...
impl ReactorLock<'_> {
    ...
    pub(crate) fn react(&mut self, timeout: Option<Duration>) -> io::Result<()> {
        ...
        let tick = self
            ...
            .wrapping_add(1);
        // let tick = unsafe {
        //     let next = self.reactor.ticker.wrapping_add(1);
        //     *(&self.reactor.ticker as *const _ as *mut _) = next;
        //     next
        // };
        self.events.clear();
        ...
        // Wake up ready tasks.
        log::info!("react: {} ready wakers", wakers.len());
        for waker in wakers {
...
impl Source {
    ...
    fn poll_ready(&self, dir: usize, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        if was_empty {
            log::info!("poll_ready poller.modify");
...
impl<T> Future for Readable<'_, T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        log::info!("readable: fd={}", self.0.handle.source.raw);
        Poll::Ready(Ok(()))
    }
}
...
impl<T> Future for ReadableOwned<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ready!(Pin::new(&mut self.0).poll(cx))?;
        log::info!("readable_owned: fd={}", self.0.handle.source.raw);
        Poll::Ready(Ok(()))
    }
}
...
impl<T> Future for Writable<'_, T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ready!(Pin::new(&mut self.0).poll(cx))?;
        log::info!("writable: fd={}", self.0.handle.source.raw);
        Poll::Ready(Ok(()))
    }
}
...
impl<T> Future for WritableOwned<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ready!(Pin::new(&mut self.0).poll(cx))?;
        log::info!("writable_owned: fd={}", self.0.handle.source.raw);
        Poll::Ready(Ok(()))
    }
}
