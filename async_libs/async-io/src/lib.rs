...
impl Stream for Timer {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        log::debug!(
            "Timer poll next thread: {}",
            std::thread::current().name().unwrap()
        );
        if Instant::now() >= self.when {
            if let Some((id, _)) = self.id_and_waker.take() {
                // Deregister the timer from the reactor.
                log::debug!("remove timer id {}", id);
                Reactor::get().remove_timer(self.when, id);
            } else {
                log::debug!("not found id and waker");
            }
            let when = self.when;
            if let Some(next) = when.checked_add(self.period) {
                self.when = next;
                log::debug!("checked_add  {:?}", self.period);
                // Register the timer in the reactor.
                let id = Reactor::get().insert_timer(self.when, cx.waker());
                self.id_and_waker = Some((id, cx.waker().clone()));
            } else {
                log::debug!("checked_add overflow");
                //let try_add_next = Instant::now().unchecked_add(self.period);
                //log::debug!("try add time");
                //return Poll::Pending;
            }
            return Poll::Ready(Some(when));
        } else {
            log::debug!("Timer waiting");
            match &self.id_and_waker {
                None => {
                    // Register the timer in the reactor.
                    log::debug!("register timer");
                    let id = Reactor::get().insert_timer(self.when, cx.waker());
                    log::debug!("got id {}", id);
                    self.id_and_waker = Some((id, cx.waker().clone()));
                }
